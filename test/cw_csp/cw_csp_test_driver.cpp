...
bool cw_csp_test_driver::test_constructor_empty(
    ...
    result &= check_condition(dut_name.str() + " vars",         set_contents_equal(&result_variables,   expected_variables));
    result &= check_condition(dut_name.str() + " constraints",  set_contents_equal(&result_constraints, expected_constraints));
